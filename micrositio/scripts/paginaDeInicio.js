...
function detectarInteraccionConContenedor(elementoSeleccionado) {
	...
	switch (elementoSeleccionado) {
		...
		case 'informes':
			...
			/*contenedor = document.getElementById(contenedoresAsociadosPorId['informes']);

			let filasOcultas = contenedor.getElementsByClassName('filasOcultas');
...
let tester = new Object();

tester.numero = 25;

tester.dimensiones = { x: 12, y: 33, z: 78}
...
let flecha = document.getElementById('flechaMostrarVariablesEconomicas');

let filasOcultas = document.getElementById('listaDeVariablesEconomicas').getElementsByClassName('elementoOculto');

flecha.addEventListener('click', ()=>{
	
	for (let i = 0; i < 2; i++) {

		filasOcultas[i].setAttribute('class', 'row text-center mt-3');
